...
class GPUQueryTestDataGenerator:
    ...
    def scenario_normal_working_hours(self) -> str:
        ...
        for hour in range(8, 20):  # 8:00-19:59
            for minute in range(0, 60, 5):  # 每 5 分鐘一筆（實際是每5秒，這裡簡化）
                # 模擬不同時段的使用率
                if hour < 10:
        ...
        return self.create_test_csv("test_normal_working_hours.csv", data)
    ...
    def scenario_off_hours_data(self) -> str:
        ...
        return self.create_test_csv("test_off_hours.csv", data)
    ...
    def scenario_high_utilization(self) -> str:
        ...
        return self.create_test_csv("test_high_utilization.csv", data)
    ...
    def scenario_multiple_gpu_ids(self) -> str:
        ...
        return self.create_test_csv("test_multiple_gpus.csv", data)
    ...
    def scenario_edge_cases(self) -> str:
        ...
        return self.create_test_csv("test_edge_cases.csv", data)

    def scenario_multi_day_range(self) -> str:
        """
        情境 6: 跨多天查詢
        ...
        for day_offset in range(0, 3):  # 3 天
            date = (datetime(2024, 11, 1) + timedelta(days=day_offset)).strftime("%Y-%m-%d")
            for hour in range(8, 20):
                for minute in range(0, 60, 15):
                    utilization = 60.0 + (day_offset * 5)  # 每天不同使用率
                    data.append({
                        'timestamp': self.generate_timestamp(date, hour, minute, 0),
                        'gpu_id': 0,
                        'utilization_gpu': utilization
                    })
        return self.create_test_csv("test_multi_day.csv", data)
    ...
    def scenario_sparse_data(self) -> str:
        ...
        return self.create_test_csv("test_sparse_data.csv", data)
    ...
    def scenario_empty_data(self) -> str:
        ...
        - 只有標題，沒有資料
        """
        return self.create_test_csv("test_empty.csv", [])
...
class TestGPUQueryAPI(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.test_files = {
            ...
            'multi_day': cls.generator.scenario_multi_day_range(),
    ...
    def test_08_empty_data_handling(self):
        ...
        # 查詢一個沒有資料的日期
        result = self._make_request("2024-12-31", "2024-12-31", 0)
